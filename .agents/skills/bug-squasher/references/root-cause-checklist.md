...
## Trust, Approval, And Payment Gates

### Symptoms
- Client displays extraction, validation, approval, or payment state that the backend did not generate
- Validation-failed invoice exposes approval or payment controls
- Run moves beyond approval unexpectedly
...
| Cause | Description | Fix |
|-------|-------------|-----|
| Trusted field from client | UI or request sets validation/approval/payment state | Ignore client value; compute server-side |
| Missing blocked branch | Invalid invoice uses normal approval component | Render an explicit blocked state |
...
- Did validation and approval pass?
