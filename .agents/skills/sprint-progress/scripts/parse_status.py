...
def aggregate_phase_statuses(
    ...
    if not str(safe_dir).startswith(str(_safe_root)):
        raise ValueError(
            "Implementation directory must be within the working directory: "
            f"{_safe_root}"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("Example: python parse_status.py backstage/development/ 1,2,3,4,5")
        sys.exit(1)
    ...
    for phase_num, statuses in phase_statuses.items():
        print(f"\nPhase {phase_num}:")
        for s in statuses:
            marker = format_status_for_table(s.status)
            print(f"  [{marker}] {s.story_id}: {s.description}")
